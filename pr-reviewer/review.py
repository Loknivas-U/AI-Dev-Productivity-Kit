#!/usr/bin/env python3
"""Run the PR reviewer against a diff file and write the review comment."""
...
DEFAULT_OUTPUT_PATH = "review_output.md"
PROMPT_PATH = Path(__file__).resolve().parent / "prompts" / "pr_review_system.md"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate an AI PR review comment from a unified diff."
    )
    ...
    parser.add_argument(
        ...
        help=f"Path to write the review comment. Defaults to {DEFAULT_OUTPUT_PATH}.",
    )
...
def write_review(output_path: Path, review_text: str) -> None:
    try:
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text(review_text, encoding="utf-8")
    except OSError as exc:
        raise RuntimeError(f"Unable to write review output at {output_path}: {exc}") from exc
...
def main() -> int:
    ...
    try:
        ...
        write_review(output_path, review_text)
